        ...
        with engine.connect() as conn:
            ...
            if inspector.has_table("heirs"):
                ...
                if "occupation" not in cols:
                    ...
                    conn.execute(
                        text("ALTER TABLE heirs ADD COLUMN occupation VARCHAR;")
                    )
            ...
            if inspector.has_table("file_registry"):
                ...
                if "status" not in cols:
                    ...
                    conn.execute(
                        text(
                            "ALTER TABLE file_registry ADD COLUMN status VARCHAR DEFAULT 'CONFIRMED';"
                        )
                    )
                ...
                if "ai_confidence" not in cols:
                    ...
                    conn.execute(
                        text(
                            "ALTER TABLE file_registry ADD COLUMN ai_confidence FLOAT DEFAULT 0.0;"
                        )
                    )
                ...
                if "extracted_data" not in cols:
                    ...
                    conn.execute(
                        text(
                            "ALTER TABLE file_registry ADD COLUMN extracted_data TEXT;"
                        )
                    )
