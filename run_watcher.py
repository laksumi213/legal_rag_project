...
class DebouncedEventHandler(FileSystemEventHandler):
    ...
    def _should_process(self, filepath):
        ...
        if filename.lower().endswith(
            (".tmp", ".crdownload", ".part", ".lock", ".ds_store", "thumbs.db")
        ):
            return False
        ...
        if len(self._processed_cache) > 1000:
            self._processed_cache = {
                k: v
                for k, v in self._processed_cache.items()
                if current_time - v < self._cooldown
            }
        return True
...
class DownloadsHandler(DebouncedEventHandler):
    def __init__(self):
        ...
        self.scanner = (
            ScannerService(inbox_path=WATCH_DIR_DOWNLOADS, processed_root=cases_root)
            if ScannerService
            else None
        )

    def _process(self, filepath):
        if not os.path.exists(filepath) or not self._should_process(filepath):
            return
        filename = os.path.basename(filepath)
        ...
        if fn_lower.endswith(valid_doc_exts):
            ...
            if any(kw in fn_lower for kw in target_keywords):
                ...
                if self.scanner:
                    try:
                        self.scanner.process_file(filepath)
                    except Exception as e:
...
class ScanHandler(DebouncedEventHandler):
    def __init__(self, inbox_path, processed_root):
        ...
        self.syncer = DataSyncEngine()
        self.service = (
            ScannerService(inbox_path, processed_root) if ScannerService else None
        )
...
class WillRAGSourceHandler(DebouncedEventHandler):
    ...
    def _is_will_document(self, filepath: Path) -> bool:
        ...
        try:
            ...
            while (
                ...
                and current_path != Z_DRIVE_PATH.parent
            ):
    ...
    def _process(self, filepath: Path):
        if not self._should_process(str(filepath)) or not filepath.exists():
            return
...
def manual_poll_recursive(handler, directory, interval=5, label="Unknown"):
    ...
    while True:
        try:
            ...
            if new_files:
                logger.info(
                    f"🔔 [手動監視:{label}] {len(new_files)}件の新規ファイルを検知"
                )
...
if __name__ == "__main__":
    ...
    if os.path.exists(WATCH_DIR_DOWNLOADS):
        ...
        threading.Thread(
            target=manual_poll_recursive,
            args=(dl_handler, WATCH_DIR_DOWNLOADS, 5, "DL"),
            daemon=True,
        ).start()
    ...
    if os.path.exists(scan_dir):
        ...
        threading.Thread(
            target=manual_poll_recursive,
            args=(scan_handler, scan_dir, 5, "NAS"),
            daemon=True,
        ).start()
