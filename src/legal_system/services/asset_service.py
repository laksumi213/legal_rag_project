...
def sync_bank_assets(
    session: Session, case_id: int, asset_data_list: List[Dict[str, Any]]
) -> Dict[str, int]:
    """
    ...
    for data in asset_data_list:
        ...
        branch = (
            _get_or_create_master(
                session, BranchMaster, bank_id=bank.id, branch_name=branch_name
            )
            if branch_name
            else None
        )
        account_type = _get_or_create_master(
            session, AccountTypeMaster, type_name=account_type_name
        )
    ...
    if ids_to_delete:
        session.query(FinancialAsset).filter(
            FinancialAsset.id.in_(ids_to_delete)
