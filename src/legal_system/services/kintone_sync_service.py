...
def get_kintone_data_as_dict(case_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if deceased:
            ...
            d_kana = format_name_full_width(
                katakana_to_hiragana(deceased.name_last_kana),
                katakana_to_hiragana(deceased.name_first_kana),
            )
...
def import_kintone_json(
    ...
    try:
        # 1. データの正規化
        k_rec_id_raw = get_value_from_keys(
            json_data, ["$id", "record_id", "レコード番号"]
        )
        k_record_id = (
            int(k_rec_id_raw) if k_rec_id_raw and k_rec_id_raw.isdigit() else None
        )

        # ★修正: 顧客コード_2 を最優先で取得
        case_num = get_value_from_keys(
            json_data, ["顧客コード_2", "顧客コード", "case_number", "案件番号"]
        )

        client_name_raw = get_value_from_keys(
            json_data, ["顧客名", "client_name", "氏名"]
        ).replace("　", " ")
        client_kana_raw = get_value_from_keys(
            json_data,
            ["顧客名(ふりがな)", "顧客名（ふりがな）", "client_name_kana", "フリガナ"],
        ).replace("　", " ")
        deceased_name_raw = get_value_from_keys(
            json_data, ["被相続人名", "deceased_name", "被相続人"]
        ).replace("　", " ")
        deceased_kana_raw = get_value_from_keys(
            json_data,
            ["被相続人名（ふりがな）", "被相続人名(ふりがな)", "deceased_name_kana"],
        ).replace("　", " ")
        sol_no = get_value_from_keys(
            json_data, ["SOL案件No.（日興）", "SOL案件No", "sol_case_number"]
        )
        intro_date = parse_all_flexible_date(
            get_value_from_keys(json_data, ["紹介日", "introduction_date"])
        )
        consent_date = parse_all_flexible_date(
            get_value_from_keys(
                json_data, ["同意書日付(日興)", "同意書日付", "consent_date"]
            )
        )
        mgr_name = get_value_from_keys(
            json_data, ["担当者①", "manager_name", "担当者1"]
        )
        opr_name = get_value_from_keys(
            json_data, ["担当者②", "operator_name", "担当者2"]
        )
        ...
        if not case:
            if case_num:
                ...
                try:
                    ...
                    temp_num = f"TMP-{datetime.now().strftime('%H%M%S')}"
            case = Case(
                case_number=temp_num,
                client_name=client_name_raw or "名称未設定",
        ...
        case.referral_sec_branch_name = get_value_from_keys(
            json_data,
            ["支店名（日興）", "支店名（大和）", "紹介元支店", "referral_branch"],
        )
        case.referral_sec_rep_name = get_value_from_keys(
            json_data,
            ["担当者（日興）", "担当者（大和）", "紹介元担当者", "referral_rep"],
        )
        ...
        start_date = parse_all_flexible_date(
            get_value_from_keys(
                json_data, ["相続開始日", "死亡日", "date_of_death", "death_date"]
            )
        )
        if start_date:
        ...
        if not contractor:
            existing_heir = (
                session.query(Heir).filter(Heir.deceased_id == deceased.id).first()
            )
            if existing_heir:
            ...
            if raw_tel:
                ...
                for i, t in enumerate(tels):
                    ...
                    if clean_tel:
                        ...
                        session.flush()
                        session.add(
                            H_ContactLink(heir_id=contractor.id, contact_id=c.id)
                        )
