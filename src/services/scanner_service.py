# src/legal_system/services/scanner_service.py
...
def find_branch_in_zengin(
    bank_code: str, branch_search_name: str
...
class DocumentHandler(ABC):
    ...
    def _generate_filename(
        ...
        return (
            f"{g_number}{client_last}様_{doc_name}{id_part}_{today_str}{ext_candidate}"
        )
    ...
    def _ensure_folder(
        self, root_path: str, keyword: str, force_name: str = None
    ) -> Optional[Path]:
        """
    ...
    def _find_target_folder(
        self, root_path: str, parent_keyword: str, target_keyword: str = None
    ) -> Optional[Path]:
        """旧ロジック互換用（必要に応じて使用）"""
    ...
    def _save_file_copy(
        self, src: Path, dest_dir: Path, filename: str
    ) -> Optional[Path]:
    ...
    def _update_or_create_registry(
        ...
        try:
            ...
            extracted_json = (
                json.dumps(analysis_data, ensure_ascii=False, default=json_serial)
                if analysis_data
                else None
            )
...
class CorporateRegistryHandler(DocumentHandler):
    ...
    def handle(
        ...
        new_filename = self._generate_filename(
            case, "商業登記簿", corp_name, original_path=original_path
        )

        # 保存先: 「取得代行資料」の中の「商業登記」または直下
        parent_dir = self._ensure_folder(case.folder_path, "取得代行資料")
        if parent_dir:
            dest_dir = self._ensure_folder(
                str(parent_dir), "商業登記", force_name="商業登記"
            )
        else:
            dest_dir = None
        ...
        if dest_dir:
            ...
            if saved_path:
                self._update_or_create_registry(
                    session,
                    case.case_id,
                    saved_path,
                    "商業登記",
                ...
                log = ContactLog(
                    ...
                    contact_content=f"【自動処理】商業登記簿({corp_name})を保存しました: {saved_path.name}",
                )
                session.add(log)
        else:
...
class TaxPaymentNoticeHandler(DocumentHandler):
    def handle(
        ...
        logger.info("💴 固定資産税・ハンドラー起動")
        new_filename = self._generate_filename(
            case, "固定資産税納税通知書", original_path=original_path
        )
        ...
        if dest_dir:
            ...
            if saved_path:
                ...
                log = ContactLog(
                    case_id=case.case_id,
                    contact_content=f"【自動処理】固定資産税納税通知書を保存しました: {saved_path.name}",
                )
                session.add(log)
        else:
            logger.warning(f"   ⚠️ 保存先フォルダが見つかりません: {case.folder_path}")
...
class BankPassbookHandler(DocumentHandler):
    def handle(
        ...
        new_filename = self._generate_filename(
            case, "通帳コピー", bank_name, original_path=original_path
        )
        ...
        if dest_dir:
            ...
            if saved_path:
                ...
                session.add(
                    ContactLog(
                        case_id=case.case_id,
                        contact_content=f"【自動処理】通帳コピーを保存しました: {saved_path.name}",
                    )
                )
        else:
            logger.warning(f"   ⚠️ 保存先フォルダが見つかりません: {case.folder_path}")
        ...
        if bank_name:
            self._register_passbook_asset(
                session, case.case_id, bank_name, branch_name, acc_type_name, acc_number
            )

    def _register_passbook_asset(
        self,
        session: Any,
        case_id: int,
        bank_name: str,
        ...
        try:
            ...
            if z_bank_code:
                bank = (
                    ...
                    .filter(BankMaster.bank_code == z_bank_code)
                    .first()
                )
            else:
                bank = (
                    session.query(BankMaster)
                    .filter(BankMaster.bank_name == bank_name)
                    .first()
                )
            ...
            if branch_name:
                ...
                if not bank.bank_code.startswith("TMP"):
                    z_br_code, z_br_name = find_branch_in_zengin(
                        bank.bank_code, branch_name
                    )
                ...
                if z_br_code:
                    ...
                    branch = (
                        ...
                        .filter(
                            BranchMaster.bank_id == bank.id,
                            BranchMaster.branch_name == branch_name,
                        )
                        .first()
                    )
                ...
                if not branch:
                    use_br_code = (
                        z_br_code if z_br_code else f"B-{uuid.uuid4().hex[:3]}"
                    )
                    use_br_name = z_br_name if z_br_name else branch_name
                    branch = BranchMaster(
                        bank_id=bank.id,
                        branch_name=use_br_name,
                        branch_code=use_br_code,
                    )
                    session.add(branch)
                    session.flush()

            ac_type = (
                session.query(AccountTypeMaster)
                .filter(AccountTypeMaster.type_name.like(f"%{acc_type_name}%"))
                .first()
            )
            if not ac_type:
...
class BalanceCertificateHandler(DocumentHandler):
    def handle(
        ...
        session: Any,
        case: Case,
        analysis_data: Union[Dict[str, Any], List[Dict[str, Any]]],
        original_path: Path,
        file_hash: Optional[str] = None,
    ) -> None:
        ...
        if isinstance(analysis_data, list):
            ...
            bank_name = first_account.get("bank_name") or first_account.get(
                "meta", {}
            ).get("bank_name", "不明銀行")
        elif isinstance(analysis_data, dict):
            accounts = [analysis_data]  # リストにラップして統一的に処理
            bank_name = analysis_data.get("bank_name") or analysis_data.get(
                "meta", {}
            ).get("bank_name", "不明銀行")
        else:
            accounts = []
        ...
        logger.info(
            f"🏦 残高証明書ハンドラー起動: {len(accounts)}件の口座情報を検出 (ファイル名用銀行: {bank_name})"
        )

        new_filename = self._generate_filename(
            case, "残高証明書", bank_name, original_path=original_path
        )

        # 保存先ロジックを修正・ログ追加
        logger.info("   -> 保存先フォルダを検索 (優先: '残高証明書')")
        dest_dir = self._find_folder(case.folder_path, "残高証明書")
        if dest_dir:
            logger.info(f"      -> '残高証明書' フォルダを発見: {dest_dir}")
        else:
            logger.info(
                "      -> '残高証明書' フォルダが見つからないため、'受領資料' フォルダを探します。"
            )
            dest_dir = self._ensure_folder(case.folder_path, "受領資料")
            if dest_dir:
        ...
        if dest_dir:
            ...
            if saved_path:
                log = ContactLog(
                    case_id=case.case_id,
                    contact_content=f"【自動処理】{bank_name}の残高証明書を保存・登録しました: {saved_path.name}",
                )
                session.add(log)
                self._update_or_create_registry(
                    session,
        ...
        for i, account_info in enumerate(accounts):
            ...
            loop_bank_name = account_info.get("bank_name") or meta.get(
                "bank_name", bank_name
            )  # 各口座の銀行名もチェック
            balance = meta.get("balance", 0)

            logger.info(
                f"   -> 口座 {i + 1}/{len(accounts)} の資産登録処理を開始: Bank='{loop_bank_name}', Balance='{balance}'"
            )
            if balance is not None:  # 残高0円でも登録
                self._upsert_asset(session, case.case_id, loop_bank_name, balance)
            else:
                logger.warning(
                    "      -> 残高が取得できなかったため、資産登録をスキップします。"
                )

    def _upsert_asset(
        self, session: Any, case_id: int, bank_name: str, balance: Any
    ) -> None:
        try:
            ...
            if z_code:
                bank = (
                    session.query(BankMaster)
                    .filter(BankMaster.bank_code == z_code)
                    .first()
                )
            else:
                bank = (
            ...
            existing = (
                session.query(FinancialAsset)
                .filter_by(case_id=case_id, bank_id=bank.id)
                .first()
            )
            if existing:
...
class SecuritiesStatementHandler(DocumentHandler):
    def handle(
        ...
        logger.info(
            f"📈 証券ハンドラー起動: {sec_company_name} (Acc: {account_number})"
        )
        logger.info(f"   -> 対象案件: {case.case_number} ({case.client_name})")
        logger.info(f"   -> 案件フォルダパス: {case.folder_path}")

        new_filename = self._generate_filename(
            case, "取引残高報告書", sec_company_name, original_path=original_path
        )
        ...
        if dest_dir:
            ...
            logger.warning("   -> 保存先フォルダが見つかりません")
        ...
        if dest_dir:
            ...
            if saved_path:
                ...
                session.add(
                    ContactLog(
                        case_id=case.case_id,
                        contact_content=f"【自動処理】{sec_company_name}の報告書を保存しました: {saved_path.name}",
                    )
                )
        else:
            logger.warning(f"   ⚠️ 保存先フォルダが見つかりません: {case.folder_path}")
        ...
        if sec_company_name:
            self._register_securities_asset(
                session,
                case.case_id,
                sec_company_name,
                branch_name,
    ...
    def _register_securities_asset(
        self,
        session: Any,
        case_id: int,
        company_name: str,
        ...
        try:
            ...
            if z_code:
                ...
                bank = (
                    session.query(BankMaster)
                    .filter(BankMaster.bank_name == company_name)
                    .first()
                )
            ...
            if branch_name:
                ...
                if not branch:
                    branch = BranchMaster(
                        ...
                        branch_code=f"B-{uuid.uuid4().hex[:3]}",
                    )
                    session.add(branch)
                    session.flush()
            ...
            ac_type = (
                session.query(AccountTypeMaster)
                .filter(AccountTypeMaster.type_name.like("%証券%"))
                .first()
            )
            if not ac_type:
...
class TransactionDetailHandler(DocumentHandler):
    def handle(
        ...
        bank_name = analysis_data.get("bank_name", "不明銀行").strip()
        new_filename = self._generate_filename(
            case, "取引明細書", bank_name, original_path=original_path
        )
...
class InvoiceHandler(DocumentHandler):
    def handle(
        ...
        new_filename = self._generate_filename(
            case, doc_label, original_path=original_path
        )
        ...
        self._update_or_create_registry(
            session, case.case_id, saved_path, doc_label, analysis_data, file_hash
        )
        ...
        existing_debt = (
            ...
            .filter(
                Liability.case_id == case.case_id,
                Liability.description.like(f"%{sender}%"),
...
class RegistryDocumentHandler(DocumentHandler):
    def handle(
        ...
        if not dest_dir:
            dest_dir = self._ensure_folder(
                case.folder_path, "不動産登記情報", force_name="不動産登記情報"
            )

        saved_path = (
            self._save_file_copy(original_path, dest_dir, final_filename)
            if dest_dir
            else None
        )
        if saved_path:
...
class OtherDocumentHandler(DocumentHandler):
    def handle(
        ...
        final_filename = self._generate_filename(
            case, doc_type, original_path=original_path
        )

        dest_dir = (
            Path(case.folder_path)
            if case.folder_path and os.path.exists(case.folder_path)
            else None
        )
        ...
        if saved_path:
            self._update_or_create_registry(
                session, case.case_id, saved_path, doc_type, analysis_data, file_hash
            )
...
class HeirListHandler(DocumentHandler):
    def handle(
        ...
        logger.info("👨‍👩‍👧‍👦 相続人リスト・ハンドラー起動")
        new_filename = self._generate_filename(
            case, "推定相続人連絡先一覧", original_path=original_path
        )
        ...
        if dest_dir:
            ...
            if saved_path:
                ...
                log = ContactLog(
                    case_id=case.case_id,
                    contact_content=f"【自動処理】推定相続人連絡先一覧を保存しました: {saved_path.name}",
                )
                session.add(log)
...
class GenericReceivedDocumentHandler(DocumentHandler):
    def __init__(
        self, db_manager: DatabaseManager, doc_name: str, target_folder_keyword: str
    ) -> None:
    ...
    def handle(
        ...
        new_filename = self._generate_filename(
            case, self.base_doc_name, original_path=original_path
        )
        ...
        if dest_dir:
            ...
            if saved_path:
                ...
                log = ContactLog(
                    case_id=case.case_id,
                    contact_content=f"【自動処理】{self.base_doc_name}を保存しました: {saved_path.name}",
                )
                session.add(log)
        else:
            logger.warning(f"   ⚠️ 保存先フォルダが見つかりません: {case.folder_path}")
...
class ScannerService:
    def __init__(
        self, inbox_path: Optional[str] = None, processed_root: Optional[str] = None
    ) -> None:
        self.inbox_path = (
            Path(inbox_path)
            if inbox_path
            else Path(os.path.join(os.path.expanduser("~"), "Downloads"))
        )
        if processed_root:
            self.processed_root = Path(processed_root)
        else:
            self.processed_root = Config.DATA_DIR / "cases"
        ...
        self.handlers = {
            ...
            "contract_document": GenericReceivedDocumentHandler(
                self.db, doc_name="契約書", target_folder_keyword="受領資料"
            ),
            "power_of_attorney": GenericReceivedDocumentHandler(
                self.db, doc_name="委任状", target_folder_keyword="受領資料"
            ),
            "agreement": GenericReceivedDocumentHandler(
                self.db, doc_name="約定書", target_folder_keyword="受領資料"
            ),
    ...
    def process_file(self, file_path: str) -> None:
        ...
        try:
            ...
            try:
                ...
                existing_entry = (
                    session.query(FileRegistry).filter_by(file_hash=f_hash).first()
                )
                if existing_entry:
                    logger.info(
                        f"   ⏭️ 既に処理済みのファイルです (Hash: {f_hash[:8]}..., Status: {existing_entry.status})"
                    )
                ...
                bank_name = analysis.get("bank_name") or analysis.get("meta", {}).get(
                    "bank_name"
                )
                if (
                    doc_type
                ...
                if is_high_confidence:
                    ...
                    logger.info(
                        f"   ✨ 高信頼度 (100%) -> 自動処理を実行します (Case: {target_case_id})"
                    )

                    # 元ファイルパスをDBに記録
                    self._register_file_entry(
                        session,
                        target_case_id,
                        path,
                        doc_type,
                    ...
                    try:
                        ...
                        if file_entry and file_entry.status == "CONFIRMED":
                            ...
                            renamed_path = self._try_rename_original_in_place(
                                session, f_hash, target_case_id, path, analysis
                            )
                            if renamed_path:
                                logger.info(
                                    f"   📁 元ファイルをリネームして保持しました: {renamed_path}"
                                )
                            else:
                                logger.info(f"   📁 元ファイルを保持しました: {path}")
                    ...
                    session.commit()
                    renamed_path = self._try_rename_original_in_place(
                        session, f_hash, candidate_id, path, analysis
                    )
                    if renamed_path:
                        logger.info(
                            f"   📁 元ファイルをリネームして保持しました: {renamed_path}"
    ...
    def _try_rename_original_in_place(
        ...
        try:
            ...
            bank_name = (
                analysis.get("bank_name")
                or analysis.get("meta", {}).get("bank_name")
                or ""
            )
            doc_name_map = {
    ...
    def process_pending_buffer(
        ...
        try:
            file_entry = (
                session.query(FileRegistry).filter_by(file_hash=buffer_id).first()
            )
            if not file_entry:
            ...
            logger.info(
                f"承認処理開始: File {file_entry.filename} -> Case ID {target_case_id}"
            )
            ...
            if file_entry.extracted_data:
                try:
                    analysis = json.loads(file_entry.extracted_data)
                except:
                    pass
            ...
            try:
                self._execute_handler(
                    session, target_case_id, analysis, file_path, file_hash=buffer_id
                )
    ...
    def ingest_will_for_rag(self, file_path: Path) -> None:
        ...
        logger.info(
            f"   ✅ [RAG Ingestion] {len(chunks)} 個のチャンクをChromaDBに格納完了: {file_path.name}"
        )
        ...
        **【C】balance_certificate / bank_passbook (銀行関連) の場合**
        ...
        msg = HumanMessage(
            content=[
                ...
                {"type": "image_url", "image_url": f"data:{mime};base64,{doc_b64}"},
            ]
        )
        try:
            ...
            if isinstance(ai_data, list) and ai_data:
                logger.warning(
                    f"   ⚠️ AIレスポンスがリスト形式でした。最初の要素を使用します。Data: {raw_content}"
                )
                ai_data = ai_data[0]
            ...
            if not isinstance(ai_data, dict):
                logger.error(
                    f"   ❌ AIレスポンスが予期せぬ形式です (辞書ではありません)。Data: {raw_content}"
                )
                return {}
        ...
        cleaned_names = [
            n.replace("様", "").replace("殿", "").strip() for n in names if n
        ]
            ...
            try:
                for name in cleaned_names:
                    hits = find_cases_by_attributes(
                        client_name=name
                    ) or find_cases_by_attributes(deceased_name=name)
                    if hits:
