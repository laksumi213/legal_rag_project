...
def render_inbox(session, gmail_service=None, scanner_service=None):
    ...
    try:
        ...
        with st.expander(
            "📥 受信トレイを確認 (未紐付け)", expanded=bool(len(pendings) > 0)
        ):
            for n in pendings:
                ...
                with st.container(border=True):
                        ...
                        if is_file:
                            ...
                            for nm in names:
                                hits = find_cases_by_attributes(
                                    client_name=nm
                                ) or find_cases_by_attributes(deceased_name=nm)
                                for h in hits:
                                    if not any(
                                        c["case_id"] == h["case_id"] for c in candidates
                                    ):
                                        candidates.append(h)
                    except Exception:
                        pass
                    ...
                    with cols_act[0]:
                        ...
                        if candidates:
                            ...
                            cand_opts = {
                                f"【{c['case_number']}】 {c['client_name']}": c[
                                    "case_id"
                                ]
                                for c in candidates
                            }
                            # デフォルトで先頭を選択
                            sel_cand_label = st.radio(
                                "紐付け先を選択",
                                list(cand_opts.keys()),
                                key=f"rad_{n.id}",
                            )
                            ...
                            st.warning(
                                "自動マッチする案件が見つかりませんでした。手動で選択してください。"
                            )
                            ...
                            case_map = {
                                f"【{c.case_number}】 {c.client_name}": c.case_id
                    ...
                    with cols_act[1]:
                                ...
                                try:
                                    ...
                                    if is_file:
                                        if scanner_service:
                                            ...
                                            success = (
                                                scanner_service.process_pending_buffer(
                                                    n.id, target_id
                                                )
                                            )
                                        else:
                                        ...
                                        success = (
                                            gmail_service.link_note_to_case_manually(
                                                n.id, target_id
                                            )
                                        )
                        ...
                        if st.button(
                            "無視", key=f"ign_{n.id}", use_container_width=True
                        ):
