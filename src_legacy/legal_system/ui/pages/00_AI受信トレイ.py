...
def render_ai_inbox():
    ...
    with db_manager._get_session() as db:
        pending_files = (
            ...
            .filter(FileRegistry.status == "PENDING")
        ...
        with col_list:
            ...
            def format_option(file_hash):
                f = next(
                    (pf for pf in pending_files if pf.file_hash == file_hash), None
                )
                if not f:
                    return "Unknown"
            ...
            if target_file.file_path and os.path.exists(target_file.file_path):
                try:
                    ...
                    mime_type = (
                        "application/pdf" if file_ext == ".pdf" else "image/jpeg"
                    )
        ...
        with col_detail:
            ...
            if target_file.case_id:
                related_case = (
                    db.query(Case).filter(Case.case_id == target_file.case_id).first()
                )
            ...
            with st.container(border=True):
                ...
                c3.metric(
                    "自動紐付案件", related_case.case_number if related_case else "不明"
                )
            ...
            is_securities_detected = (
                "証券" in extracted_bank_name or "證券" in extracted_bank_name
            )
            ...
            elif ai_detected_type == "securities_statement" or (
                is_securities_detected and ai_detected_type == "balance_certificate"
            ):
                # 証券モード優先
                default_key = "証券・金融商品 (明細あり)"
            elif ai_detected_type in [
                "balance_certificate",
                "bank_passbook",
                "transaction_detail",
            ]:
            ...
            case_options = {
                c.case_id: f"{c.case_number} {c.client_name}" for c in cases
            }
            default_idx = 0
                ...
                with st.container(border=True):
                    ...
                    with qc1:
                        if target_case_obj.kintone_record_id:
                            ...
                            st.button(
                                "🔗 連携なし", disabled=True, use_container_width=True
                            )

                    with qc2:
                        path_val = target_case_obj.folder_path or ""
                        col_path_in, col_open_btn = st.columns([3, 1])
                        new_path = col_path_in.text_input(
                            "Path",
                            value=path_val,
                            label_visibility="collapsed",
                            key=f"fp_{target_file.file_hash}",
                        )
                        if col_open_btn.button(
                            "📂 開く",
                            key=f"btn_open_{target_file.file_hash}",
                            use_container_width=True,
                        ):
                            if new_path:
                                ...
                                if new_path != path_val:
                                    update_case_folder_path(
                                        target_case_obj.case_id, new_path
                                    )
                                    st.rerun()
                        if new_path != path_val:
            ...
            if current_doc_type == "securities_statement":
                ...
                with col_s1:
                    sec_name = st.text_input(
                        "証券会社名",
                        value=ai_data.get("bank_name", ""),
                        key=f"sec_n_{target_file.file_hash}",
                    )
                    sec_branch = st.text_input(
                        "本支店名",
                        value=meta.get("branch_name", ""),
                        key=f"sec_b_{target_file.file_hash}",
                    )
                with col_s2:
                    sec_acc = st.text_input(
                        "口座番号 (英数可)",
                        value=meta.get("account_number", ""),
                        key=f"sec_a_{target_file.file_hash}",
                    )
                    ...
                    # valueにint型を渡す
                    sec_total = st.number_input(
                        "合計評価額 (円)",
                        value=sec_total,
                        format="%d",
                        key=f"sec_t_{target_file.file_hash}",
                    )
                ...
                try:
                    ...
                    if "valuation" in df_holdings.columns:
                        df_holdings["valuation"] = (
                            pd.to_numeric(df_holdings["valuation"], errors="coerce")
                            .fillna(0)
                            .astype(int)
                        )
                except Exception as e:
                ...
                edited_holdings = st.data_editor(
                    ...
                    column_config={
                        "name": st.column_config.TextColumn(
                            "銘柄名 (ファンド名)", required=True, width="large"
                        ),
                ...
                if st.button(
                    ...
                    svc.process_pending_buffer(
                        ...
                        selected_case_id,
                        override_doc_type="securities_statement",
                    )
            ...
            # B. 推定相続人一覧
                ...
                edited_heirs_df = st.data_editor(
                    ...
                    column_config={
                        ...
                        "honseki": st.column_config.TextColumn(
                            "本籍地", width="medium"
                        ),
                        ...
                        try:
                            ...
                            for index, row in edited_heirs_df.iterrows():
                                ...
                                add_heir(
                                    ...
                                    phone_contacts=[{"value": row.get("phone")}]
                                    if row.get("phone")
                                    else [],
                                )
                                count += 1
                            ...
                            svc.process_pending_buffer(
                                target_file.file_hash,
                                selected_case_id,
                                override_doc_type="heir_list",
                            )

                            st.success(
                                f"{count}名の相続人を追加し、書類を登録しました！"
                            )
                            st.rerun()
                        except Exception as e:
                            st.error(f"登録エラー: {e}")
            ...
            # C. 固定資産税 (保存のみ)
                ...
                if st.button(
                    ...
                    success = svc.process_pending_buffer(
                        target_file.file_hash,
                        selected_case_id,
                        override_doc_type="tax_payment_notice",
                    )
            ...
            # E. 銀行・預金 (デフォルト)
                ...
                with col_f1:
                    edited_vals["bank_name"] = st.text_input(
                        "銀行名",
                        value=ai_data.get("bank_name", ""),
                        key=f"bn_{target_file.file_hash}",
                    )
                    edited_vals["branch_name"] = st.text_input(
                        "支店名",
                        value=meta.get("branch_name", ""),
                        key=f"br_{target_file.file_hash}",
                    )
                ...
                with col_f2:
                    ...
                    edited_vals["balance"] = st.number_input(
                        "金額/残高", value=val_bal, key=f"bal_{target_file.file_hash}"
                    )
                    edited_vals["account_number"] = st.text_input(
                        "口座番号",
                        value=meta.get("account_number", ""),
                        key=f"an_{target_file.file_hash}",
                    )
                ...
                with b1:
                    if st.button(
                        ...
                        type="primary",
                        use_container_width=True,
                        key=f"ok_{target_file.file_hash}",
                    ):
                        ...
                        target_file.extracted_data = json.dumps(
                            ai_data, ensure_ascii=False
                        )
                        db.commit()
                        ...
                        svc.process_pending_buffer(
                            target_file.file_hash,
                            selected_case_id,
                            override_doc_type=current_doc_type,
                        )
            ...
            with b2 if "b2" in locals() else st.container():
                if st.button(
                    "🗑️ 除外する",
                    use_container_width=True,
                    key=f"del_{target_file.file_hash}",
                ):
