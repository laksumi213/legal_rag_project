...
def search_cases_simple(session, keyword: str):
    ...
    return (
        base_query.filter(
            Case.case_number.ilike(clean_key) | Case.client_name.ilike(clean_key)
        )
        .limit(10)
        .all()
    )
...
def main():
    ...
    with st.sidebar:
        ...
        # 選択肢辞書の作成
        case_options = {
            f"{c.case_number}: {c.client_name}": c.case_id for c in filtered_cases
        }
    ...
    tab_notary, tab_rag = st.tabs(
        ["⚖️ 公証役場・アクセス (Logistics)", "📚 銀行RAG・ナレッジ (Knowledge)"]
    )
    ...
    with tab_notary:
        ...
        st.caption(
            "Geminiが住所から最寄りの公証役場を推論し、アクセス方法を提案します。"
        )
        ...
        if current_case.deceased_ref and current_case.deceased_ref.heirs:
            contractor = next(
                (h for h in current_case.deceased_ref.heirs if h.is_contracting_party),
                None,
            )
            if contractor:
                addr_link = (
                    session.query(H_AddressHistory)
                    .filter_by(heir_id=contractor.id, is_current_address=True)
                    .first()
                )
                if addr_link:
        ...
        target_addr = col_in.text_input(
            "検索起点（依頼者住所など）", value=origin_address, key="notary_search_addr"
        )
    ...
    with tab_rag:
        ...
        if not rag_service:
            ...
            query = st.text_input(
                "質問・検索キーワード",
                placeholder="例: 三菱UFJの残高証明に必要な書類は？ / 鈴木一郎の戸籍",
            )
                    ...
                    if docs:
                        for doc in docs:
                            with st.expander(f"📄 {doc['filename']}"):
                                st.caption(
                                    f"登録日: {doc['registered_at']} | 種別: {doc['doc_type']}"
                                )

                                # --- 多機能PDFビューア表示 ---
                                pdf_path = os.path.join(
                                    ROOT_DIR, "data", "demo_bank_docs", doc["filename"]
                                )
                                if os.path.exists(pdf_path):
                                    try:
                                        ...
                                        st.error(
                                            f"プレビュー生成中にエラーが発生しました: {e}"
                                        )
                                else:
                                    st.warning(
                                        f"ファイルが見つかりません: {doc['filename']}"
                                    )
                    else:
