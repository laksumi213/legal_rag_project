...
from legal_system.ui.excel_generator import fill_initial_set_excel
...
def show_document_creation_page():
    ...
    with st.expander("📂 Excelテンプレート設定", expanded=False):
        st.info(
            "デフォルトではサーバー内の最新版テンプレートが使用されます。手元のファイルを修正して使いたい場合のみアップロードしてください。"
        )
    ...
    if st.button("解析・Excel作成実行", type="primary"):
        ...
        try:
            ...
            # ファイル名の生成（顧客名を含める）
            customer_name = (
                data.get("顧客名", "未設定").replace("　", "").replace(" ", "")
            )
            ...
            st.error(
                "JSON形式の読み込みに失敗しました。コピー内容が正しいか確認してください。"
            )
