...
def main():
    ...
    if is_inheritance:
        st.caption(
            "被相続人の出生から死亡までの戸籍が繋がっているか（連続性）を可視化・チェックします。"
        )
    else:
    ...
    with st.container(border=True):
        ...
        c1.markdown(
            f"**{target_role_label}**: {target_person.name_last} {target_person.name_first}"
        )
        dob_str = (
            convert_seireki_to_wareki(target_person.date_of_birth)
            if target_person.date_of_birth
            else "未登録"
        )
        ...
        if is_inheritance:
            dod_str = (
                convert_seireki_to_wareki(target_person.date_of_death)
                if target_person.date_of_death
                else "未登録"
            )
            c3.markdown(f"**死亡日**: {dod_str}")
            if not target_person.date_of_death:
                st.info(
                    "ℹ️ 死亡日が未登録です。除籍謄本等をアップロードすると自動登録されます。"
                )
    ...
    with col_L:
        ...
        if target_person and target_person.name_last:
            ...
            st.caption(
                f"💡 AIへのヒント: 名字は「{hint_family_name}」と想定して読み取ります。"
            )

        label = (
            "戸籍謄本・除籍謄本・原戸籍" if is_inheritance else "戸籍謄本（現在戸籍）"
        )
        ...
        if uploaded_files:
            ...
            if files_to_process:
                with st.status(
                    f"🚀 {len(files_to_process)}件を一括解析中...", expanded=True
                ) as status:
                    for i, file_obj in enumerate(files_to_process):
                        st.write(
                            f"📄 [{i + 1}/{len(files_to_process)}] 解析中: {file_obj.name}"
                        )
    ...
    with col_R:
        ...
        if not records:
            ...
            if timeline_data:
                ...
                chart = (
                    ...
                    .encode(
                        ...
                        x2="End:T",
                        y=alt.Y(
                            "Type:N", title="種類", sort=["【必要期間】出生〜死亡"]
                        ),
                        color=alt.Color("Type:N", legend=None),
                        tooltip=["Label", "Start", "End"],
                    )
                    .properties(title="戸籍の取得状況タイムライン", height=200)
                    .interactive()
                )
            ...
            if (
                is_inheritance
                and target_person.date_of_birth
                and target_person.date_of_death
            ):
                gaps, advices = service.check_continuity_gaps(target_person.id)

                if not gaps:
                    st.success(
                        "🎉 おめでとうございます！出生から死亡まで連続しています。"
                    )
                else:
                    ...
                    # AIによる次の一手アドバイス
                    if st.button(
                        "🤖 空白を埋めるためのアクションをAIに聞く", type="primary"
                    ):
                        with st.spinner("AIが不足箇所を分析し、請求先を推論中..."):
                            suggestion = service.recommend_missing_koseki_action(
                                target_person.id, gaps
                            )
            ...
            st.markdown("##### 📝 データの修正")
            st.caption(
                "AIの誤読がある場合、下表を直接編集して「修正保存」を押してください。"
            )
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    "id": None,
                    "書類種類": st.column_config.SelectboxColumn(
                        "種類", options=["現在戸籍", "除籍謄本", "改製原戸籍", "住民票"]
                    ),
                    ...
                    "開始日": st.column_config.DateColumn(
                        "開始日", format="YYYY/MM/DD"
                    ),
                    "終了日": st.column_config.DateColumn(
                        "終了日", format="YYYY/MM/DD"
                    ),
                },
                use_container_width=True,
                num_rows="dynamic",
                            ...
                            if pd.notna(rec_id):
                                ...
                                if record:
                                    ...
                                    if pd.notnull(row["開始日"]):
                                        record.valid_from = (
                                            row["開始日"].date()
                                            if hasattr(row["開始日"], "date")
                                            else row["開始日"]
                                        )
                                    if pd.notnull(row["終了日"]):
                                        record.valid_to = (
                                            row["終了日"].date()
                                            if hasattr(row["終了日"], "date")
                                            else row["終了日"]
                                        )
            ...
            with col_clear:
                if st.button("全データをクリアする"):
                    if target_type == "deceased":
                        session.query(FamilyRegister).filter_by(
                            deceased_id=target_person.id
                        ).delete()
                    else:
                        session.query(FamilyRegister).filter_by(
                            heir_id=target_person.id
                        ).delete()
                    session.commit()
                    st.session_state["processed_koseki_ids"] = set()
                    st.rerun()
