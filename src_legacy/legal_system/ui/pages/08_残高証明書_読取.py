...
def normalize_ai_result(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    doc_type = (
        str(data.get("type") or data.get("institution_type") or "").strip().upper()
    )
    if not doc_type:
        # holdingsがあればSECURITY優先
        doc_type = (
            "SECURITY"
            if isinstance(data.get("holdings"), list) and data.get("holdings")
            else "BANK"
        )
...
def save_bank_assets_to_db(session: Session, case_id: int, data: Dict[str, Any]) -> int:
    """銀行口座データをDBに保存"""
    raw_bank_name = str(
        data.get("institution_name") or data.get("bank_name") or ""
    ).strip()
    bank = None

    bank = (
        session.query(BankMaster).filter(BankMaster.bank_name == raw_bank_name).first()
    )
    ...
    if not bank:
        ...
        if z_code:
            bank = (
                session.query(BankMaster).filter(BankMaster.bank_code == z_code).first()
            )
            if not bank:
                ...
                session.flush()
                st.toast(
                    f"ℹ️ 全銀データから銀行マスタを登録しました: {z_name} ({z_code})",
                    icon="🏦",
                )
    ...
    for acc in data.get("accounts", []):
        ...
        if raw_branch_name:
            ...
            if not branch and not bank.bank_code.startswith("TMP"):
                bz_code, bz_name = find_branch_in_zengin(
                    bank.bank_code, raw_branch_name
                )
                if bz_code:
                    branch = (
                        session.query(BranchMaster)
                    ...
                    if not branch:
                        branch = BranchMaster(
                            bank_id=bank.id, branch_name=bz_name, branch_code=bz_code
                        )
                        session.add(branch)
                        session.flush()
...
# 5. DB保存ヘルパー - 証券会社用
# ==========================================
def save_security_assets_to_db(
    session: Session, case_id: int, data: Dict[str, Any]
) -> int:
    """証券会社の銘柄データをDBに保存"""
    ...
    securities_company = (
        ...
        .filter(BankMaster.bank_name == raw_company_name)
        .first()
    )
    ...
    file_reg = (
        session.query(FileRegistry).filter(FileRegistry.file_hash == file_hash).first()
    )
...
def main() -> None:
    ...
    with col_left:
        ...
        if uploaded_file:
            ...
            if (
                st.session_state.get("last_uploaded_file_identifier")
                != current_file_identifier
            ):
                with st.spinner("書類の内容をAIが解析しています..."):
                    result = analyze_balance_cert_with_ai(
                    ...
                    if "error" in result:
                        ...
                        st.success(
                            "AI解析が完了しました。右側で結果を確認・編集してください。"
                        )

                st.session_state["last_uploaded_file_identifier"] = (
                    current_file_identifier
                )
                st.rerun()
    ...
    with col_right:
        ...
        if "zandaka_result" in st.session_state:
                ...
                if institution_type == "SECURITY":
                    ...
                    edited_df = st.data_editor(
                        ...
                        column_config={
                            ...
                            "asset_category": st.column_config.TextColumn(
                                "資産区分", width="small"
                            ),
                        },
                        num_rows="dynamic",
                    ...
                    edited_df = st.data_editor(
                        df,
                        column_config={
                            "branch_name": st.column_config.TextColumn(
                                "支店名", width="medium"
                            ),
                ...
                if st.form_submit_button("💾 データベースに登録"):
                    try:
                        if institution_type == "SECURITY":
                            ...
                            count = save_security_assets_to_db(
                                session, target_case_id, final_data
                            )
                            session.commit()
                            st.toast(
                                f"登録完了: {count}銘柄の証券資産を保存しました！",
                                icon="✅",
                            )
                        else:
                            final_data = {
                            ...
                            count = save_bank_assets_to_db(
                                session, target_case_id, final_data
                            )
                            session.commit()
                            st.toast(
                                f"登録完了: {count}件の資産を保存しました！", icon="✅"
                            )
            ...
            if os.path.exists(zengin_path):
                st.caption(
                    f"✅ 全銀データ連携: 有効 ({len(json.load(open(zengin_path, encoding='utf-8')))}行)"
                )
            else:
                st.caption(
                    "⚠️ 全銀データが見つかりません (自動更新スクリプトを実行してください)"
                )
